...
def render_progress_card(state: dict) -> None:
    ...
    st.markdown(
        f"""
        <div class="glass-card progress-shell">
...
def render_sidebar_snapshot(state: dict) -> None:
    ...
    st.markdown(
        ...
        <div class="glass-card">
            <div class="section-title">Patient Info Card</div>
            <span class="metric-chip">Age: {html.escape(str(age))}</span>
            <span class="metric-chip">Gender: {html.escape(str(gender))}</span>
            <span class="metric-chip">Stage: {html.escape(str(state.get("step", "unknown").title()))}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )

    complaint_text = html.escape(
        state.get("complaint") or "The patient complaint will appear here once intake begins."
    ).replace("\n", "<br>")

    st.markdown(
        f"""
        <div class="glass-card">
            <div class="section-title">Complaint Card</div>
            <div class="subtle-text">{complaint_text}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    steps = [
        ...
        (
            "2",
            "Symptom Details",
            state.get("duration") or "We are gathering when the symptoms started and the care context",
        ),
        ("3", "Follow-up Questions", current_question or "Guided questions will appear here one at a time"),
        (
            "4",
            "Medical History",
            "Medications, allergies, and long-term conditions are collected near the end",
        ),
    ]
    ...
    for number, title, text in steps:
        ...
        steps_html += f"""
            <div class="step-card">
                <div class="step-label">Step {html.escape(number)}</div>
                <strong>{safe_title}</strong><br>
                <span class="subtle-text">{safe_text}</span>
            </div>
        """
    ...
    st.markdown(
        f"""
        <div class="glass-card">
            <div class="section-title">Question Flow UI</div>
            {steps_html}
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with left_col:
        with st.container():
            st.markdown(
                """
                <div class="glass-card">
                    <div class="section-title">Begin New Intake</div>
        ...
        if state["step"] == "clarification":
            with st.container():
                st.markdown(
                    """
                    <div class="glass-card">
                        <div class="section-title">Optional Prescription Upload</div>
                    """,
                    unsafe_allow_html=True,
                )
...
def render_final_report(chat: ChatController, state: dict) -> None:
        ...
        with st.container():
            st.markdown(
                ...
                <div class="urgency-banner {urgency_class}">
                    Urgency Level: {html.escape(str(urgency))}
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        with st.container():
            st.markdown(
                ...
                <div class="report-card">
                    <div class="section-title">Professional Clinical Report</div>
    ...
    with right_col:
        ...
        complaint_text = html.escape(state.get("complaint") or "No complaint recorded.").replace("\n", "<br>")
        st.markdown(
            f"""
            <div class="glass-card">
                <div class="section-title">Complaint Card</div>
        ...
        for label, value in history_items:
            history_html += f"""
                <div class="history-item">
                    ...
                    <span class="subtle-text">{html.escape(str(value)).replace("\n", "<br>")}</span>
                </div>
            """
